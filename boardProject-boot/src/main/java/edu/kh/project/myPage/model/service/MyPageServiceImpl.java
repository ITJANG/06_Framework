...
public class MyPageServiceImpl implements MyPageService{
    ...
    MyPageServiceImpl(MainController mainController) {
	...
	public int changePw(Map<String, String> paramMap, int memberNo) {
		...
		// DB에 업데이트
		// SQL 전달해야하는 데이터 2개 (암호화한 새 비번 encPw, 회원번호 memberNo)
		// -> mapper에 전달할 수 잇는 전달인자는 1개
		// -> 묶어서 전달 (paramMap 재활용)
		
		paramMap.put("encPw", encPw);
		paramMap.put("memberNo", memberNo + ""); // 문자열 타입으로 변경 : + ""
	...
	public int fileUpload2(MultipartFile uploadFile, int memberNo) throws Exception {
		...
		//      2) method chaining을 이용하여 한줄로 작성 가능
		// 		3) 매개변수 생성자 불필요
		...
		int result = mapper.inserUploadFile(uf);
		
		// 4. 삽입 성공 시 파일을 지정된 서버 폴더에 저장
		if(result == 0) return 0;
		
		// folderPath 경로( C:/uploadFiles/test/변경된파일명) 으로 서버 pc에 저장
