...
public class EamilServiceImpl implements EmailService {
	...
	public String sendEmail(String htmlName, String email) {
		...
		// 2. DB 저장이 성공된 경우에만 메일 발송 시
		MimeMessage mimeMessage = mailSender.createMimeMessage();
		// 메일 발송 시 사용하는 객체
		
		// 메일 발송을 도아줘는 Helper 클래스 (파일첨부, 템플릿 설정 등 쉽게 처리)
		try {
			MimeMessageHelper helper = new MimeMessageHelper(mimeMessage, true, "UTF-8");
			// - mimeMessage : MimeMessage 객체로, 이메일 메시지의 내용을 담고 있음
			// 					이메일의 본문, 제목, 발송자, 수신자 정보 등 포함
	...
	private String loadHtml(String authKey, String htmlName) {
		...
		context.setVariable("authKey", authKey); // 템플릿에서 사용할 변수 authKey에 값
