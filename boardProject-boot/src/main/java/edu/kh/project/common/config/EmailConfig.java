...
public class EmailConfig {
	/*
	 * 이메일 인증 절차 구현 1. 관련 테이블 생성 2. 구글 SMTP 세팅(구글에서 비밀번호 발급) 3. 구글 SMTP를 나의 Java
	 * 어플리케이션에서 이용할 수 있도록 Config 세팅 SMTP(Simple Mail Transfer Protocol) : 이메일을 전송하는데
	 * 사용되는 표준 통신 프로토콜 동작 방식 1. Spring boot 어플리케이션에서 내가 이메일 보냄 2. Google SMTP 서버로 전송
	 * (Google 서버가 중간 배달자 역할) 3. Google SMTP 서버가 이메일주소에 맞는 “수신자”에게 이메일 서버 (gmail,
	 * yahoo, 회사 이메일 서버..)를 통해 발송 4. 최종 수신자 이메일을 받음 5. 기본 포트 25를 사용하지만, 보안 강화한 포트
	 * 587이나 465가 자주 사용됨
	 */

	// @Value : properties에 작성된 내용 중 key가 일치하는 value값을 얻어와 필드에 삽입
	...
	public JavaMailSender javaMailSender() {
		 ...
		 * 
		 * JavaMailSender : 이메일을 보내는데 사용되는 인터페이스로, JavaMailSenderImpl 클래스를 통해 구현됨 SMTP
