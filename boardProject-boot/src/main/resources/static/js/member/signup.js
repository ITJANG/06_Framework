...
sendAuthKeyBtn.addEventListener("click", () => {
  ...
  // 새로 발급받은 인증번호 확인전까지는 checkObj.authKey 는 false
  checkObj.authKey = false;
  // 인증번호 발급 관련된 메시지 지우기
  authKeyMessage.innerText = "";
  ...
  authKeyMessage.classList.remove("confirm", "error");// 05:00 세팅
...
// 매개변수 전달받은 숫자가 10 미만인 경우(한자리) 앞에 0붙여서 반환
...
checkAuthKeyBtn.addEventListener("click", () => {
  ...
  if (min === 0 && sec === 0) { // 타이머가 00:00인 경우
    alert("인증번호 입력 제한 시간을 초과하셨습니다. 다시 발급해주세요.")
    return;
  }

  if (authKey.value.length < 6 || authKey.value.length >= 7) { // 인증번호가 제대로 입력이 안된 경우 (길이가 6미만 또는 7이상)
...
const checkPw = () => {
  ...
  if (memberPw.value === memberPwConfirm.value) {
    ...
    checkObj.memberPwConfirm = true; // 비밀번호 확인 ture
    return;
  }
