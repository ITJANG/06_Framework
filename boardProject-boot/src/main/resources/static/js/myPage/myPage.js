...
if(updateInfo != null) {
    ...
    updateInfo.addEventListener("submit", async e => {
        ...
        submit 되어버린 다음 일 수 있음
        -> 비동기 작업중에 form의 기본 제출 동작 일어날 가능성이 생김
        */
    });
}
...
if(profileForm != null){
    ...
    imageInput.addEventListener("change", () => {
        ...
        if(file) { // 파일이 선택된 경우
            ...
            if(file.size <= MAX_SIZE) { // 현재 선택한 파일의 크기가 허용범위 이내인 경우
                ...
                // createObjectURL : 웹이서 접근 가능한 임시 URL 반환
            ...
            // 이전 선택한 파일로 복원 (input)
            if(previousFile != null){ // 이전 파일이 존재 한다면
                const dataTransfer = new DataTransfer();
                ...
                // --> input 태그의 files 속성은 File:ist 만 저장 가능한 형태이기 때문에
    ...
    deleteImage.addEventListener("click", () => {
        ...
        if(profileImg.src !== defaultImageUrl) {
            ...
            // 기본 이미지 상태에서 삭제버튼 클릭시 상태를 변경하지 않음
