...
VALUES (
  SEQ_MEMBER_NO.NEXTVAL, 'test@kh.com',
  '$2a$10$6Dqg9zvW0ITe8MEZKybRoO2zw5MDr6Vp/.rMCCMm82tDdk8O7J1Ce', -- pass02!
  '테스트', '01012345678', '서울시', NULL, '1',
  SYSDATE, 'N'
);
...
UPDATE "MEMBER" SET
MEMBER_PW = '$2a$10$6xkIYavWh4KxI8tLEMBKbeGCB3i5Pcs2yghF0qL5.VRtRQ1c9gvjW'
WHERE MEMBER_NO = 1;
...
VALUES(6, 1998); -- 1번 회원이 1998번 게시글에 좋아요를 누름
...
-- 작성일 : 몇 초/ 몇 분/ 몇 시간 전 조회, 하루 넘어가면 YYYY-MM-DD 형식 조회
...
SELECT CHATTING_ROOM_NO
	...
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
		...
		R.PARTICIPANT, -- NULL이 아닌경우 (로그인한 사람 오픈멤버일때는 참가자 타겟 번호)
		R.OPEN_MEMBER	 -- NULL인 경우 (내가 오픈멤버가 아닐때는 OPEN_MEMBR)
		) TARGET_NO	
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
		WHERE R2.CHATTING_ROOM_NO = R.CHATTING_ROOM_NO
		AND R2.OPEN_MEMBER = 1),
...
COUNT(*) AS MEMBER_COUNT, ENROLL_DATE
FROM MEMBER
