...
-> 어떤 결과 일지는 모르지만 반드시 결과를 보내주겠다는 약속
...
function getTotalCount() { // 함수 정의
  ...
  .then( result => { // 첫번째 콜백함수가 완료된 후 호출되는 콜밸 함수
...
const selectTodoList = () => {
  ...
  .then(todoList => {
      ...
      // resp.text() 사용 시 문자열(JSON이 그대로 노출)
      ...
      for(let todo of todoList) { // 향상된 for문
        ...
        for(let key of arr) {
          ...
          if(key === 'todoTitle') {
            ...
            // a태그 클릭 시 페이지 이동 막기(비동기 요청 사용 위해)
          ...
          tr.append(td); // tr 의 마지막요소 현재 td 추가하기
...
deleteBtn.addEventListener("click", () => {
  ...
  fetch("/ajax/delete", {
    ...
    body : todoNo // todoNo 단일 값 하나는 JSON 형태로 자동 변환되어 전달
    // body : JSON.stringify(todoNo)
    // -> 원래는 이렇게 명시 하는게 옳음(엄격한 환경에서는 꼭 명시)
  })
  .then(resp => resp.text())
  .then(result => {
    console.log(result);
...
changeComplete.addEventListener("click", () => {
  ...
  .then(result => {
    ...
    if(result > 0) { // 성공
      ...
      selectTodoList();
      // 서버 부하 줄이기 가능하지만 코드가 복잡해서 오히려 비용증가
      // 그냥 서버 요청 함수 호출로 해결
